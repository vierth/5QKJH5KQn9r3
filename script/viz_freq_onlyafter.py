...
with open("../data/corpus/.txt", "r", encoding="utf8") as rf:
    jpm_text = rf.read()
...
for d in data:
    loc = int(d[0])
    locations = d[1].split(",")
    if "after" in set(locations):
        vals[loc] = locations.count("after")
...
fig.update_layout(title_text='Heatmaps of Shared Text in <i>Jinpingmei</i> by Chapter in Just Later Texts', coloraxis = {'colorscale':'blackbody'})
