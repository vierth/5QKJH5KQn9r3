...
for d in data:
    loc = int(d[0])
    locations = d[1].split(",")
    if "before" in set(locations):
        vals[loc] = locations.count("before")
...
fig.update_layout(title_text='Heatmaps of Shared Text in <i>Jinpingmei</i> by Chapter in Earlier Texts', coloraxis = {'colorscale':'blackbody'})
