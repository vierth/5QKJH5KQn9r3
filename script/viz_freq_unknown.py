...
with open("char_res_status.txt",'r',encoding='utf8') as rf:
    text = rf.read().split("\n")
...
for d in data:
    ...
    unique_loc = set(locations)
    if "before" not in unique_loc:
        if "blank" in unique_loc or "unknown" in unique_loc:
            vals[loc] = len([d for d in locations if d != "after"])

    # if len(set(locations)) == 1 and "unknown" in set(locations):
    #     vals[loc] = len(locations)
    # elif len(set(locations)) == 1 and "blank" in set(locations):
    #     vals[loc] = len(locations)
    # elif len(set(locations)) == 2 and "blank" in set(locations) and "unknown" in set(locations):
    #     vals[loc] = len(locations)
...
fig.update_layout(title_text='Heatmaps of Shared Text in <i>Jinpingmei</i> by Chapter in Unknown Texts', coloraxis = {'colorscale':'blackbody'})
